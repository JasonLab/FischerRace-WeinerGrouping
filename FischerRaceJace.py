...
Rh_Pheno= input("Please enter the antigens that were positive. So for example if you got D+C+c-E+e+ you would enter DCEe. If D is neg don't enter d:  ")
...
Wiener_Orders=[]
ESChecker = 0
MaxPos=[] #Does not need to be list?
EqualScore=[]
...
for x in Wiener_Group: #Gives a score to each genotype based on the orders of classification
    ...
    if 'rII' in x:
        Wiener_Score = Wiener_Score - 1
    Wiener_Orders.append(Wiener_Score)
#print(Wiener_Orders) #Important checkpoint until I'm certain classification is fixed
MaxPos= (Wiener_Orders.index(max(Wiener_Orders)))
#print(MaxPos)
FirstNumberOne= Wiener_Group[MaxPos]
...
print("The most likely genotype(s) are: ")
